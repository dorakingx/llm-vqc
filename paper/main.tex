...
\begin{equation}
F_{\rm trash}(\theta)=\frac{1}{N}\sum_{j=1}^{N}\Pr_{\theta,\psi(h_j)}(q_2q_3=00),
\end{equation}
...
This is the same basic compression principle as the original QAE \cite{romero2017}.
...
The exact prompt is reproduced in Appendix~\ref{app:prompt} and the complete proposal pool is frozen in the experiment source and exported as JSON by the reproduction script.
...
Against the stronger RY-only topology control, the paired mean gain is $+0.0203$ with bootstrap 95\% interval $[0.0114,0.0294]$.
...
\begin{quote}
Compress ground states of the 4-qubit open-chain transverse-field Ising Hamiltonian $H=-\sum Z_iZ_{i+1}-h\sum X_i$, $h\in[0.2,2.0]$, into latent qubits $q_0,q_1$ while trash qubits $q_2,q_3$ should end in $\lvert00\rangle$.
